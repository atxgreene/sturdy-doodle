...
> **Historical draft from v0.2 (April 9 2026).** Repo references use the
> original name `atxgreene/sturdy-doodle`; the canonical repo is now
> `atxgreene/Mnemosyne`. Tier-count and feature claims in this article
> reflect v0.2 state, not the current v0.9+ architecture. For the
> current launch piece see
> [`v0.8-launch-substack.md`](./v0.8-launch-substack.md). Kept unedited
> as a dated artifact of the early framing.
